...
// In production behind the same domain, '/api' works via reverse proxy; override with VITE_API_URL when needed
  ...
  getUsers: (params?: any) => api.get('/admin/users', { params }).then((r) => ({
    data: {
      users: (r.data.users || []).map((u: any) => ({
        ...
        personalPoints: u.personalPoints || 0, // Исправлено: используем personalPoints из backend
        points: u.points || 0,
        admin_points: u.admin_points || 0,
        role: u.role,
